...
pub const Value = union(enum(u8)) {
    ...
    pub fn getString(this: Value, key: []const u8) ?[]const u8 {
        if (this != .object) return null;
        return this.object.getS(key);
    }

    pub fn getInt(this: Value, key: []const u8) ?i64 {
        if (this != .object) return null;
        if (this.object.get(key, .number)) |n| {
            return n.get(i64);
        }
        return null;
    }

    pub fn getBool(this: Value, key: []const u8) ?bool {
        if (this != .object) return null;
        return this.object.getB(key);
    }
};
...
pub fn stringify(writer: anytype, value: anytype, options: std.json.StringifyOptions) !void {
    _ = options;
